...
if( 'querySelector' in document
    ...
    window.onload = function() {
        ...
        function saturatedTones(imageData) {
            ...
            var saturateds = saturationData.arr.slice(Math.floor(saturationData.arr.length / 2));
        ...
        function renderResults(colours, canvas) {
            var swatch;
            var i;
			var hexes = [];
            ...
            for (i = 0; i < colours.length; i += 1) {
				...
				var hex = rgbToHex(colours[i]);
				hexes.push(hex);
				swatchValue.innerHTML = hex;
			...
			area.className = "export__area";
			area.innerHTML = hexes.join("\n");
			exportEl.appendChild(area);
        }
